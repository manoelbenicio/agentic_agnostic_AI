...
@pytest.mark.asyncio
async def test_techlead_fanout_inherits_parent_seat_without_extra_pool_acquire() -> None:
    pool = SeatPool()
    parent = Seat("lead-seat", "tenant-1", "codex", "/tmp/lead")
...
@pytest.mark.asyncio
async def test_low_autonomy_requires_human_approval_before_spawn() -> None:
    pool = SeatPool()
    parent = Seat("lead-seat", "tenant-1", "codex", "/tmp/lead")
    pool.register_seat(parent)
    parent_seat = await pool.acquire("tenant-1", "codex")
...
@pytest.mark.asyncio
async def test_scheduler_quota_throttles_excess_fanout_to_queue() -> None:
    pool = SeatPool()
    parent = Seat("lead-seat", "tenant-1", "codex", "/tmp/lead")
    pool.register_seat(parent)
    parent_seat = await pool.acquire("tenant-1", "codex")
