...
def test_tasks_dispatch_socket_and_terminal_modes(api_client):
    ...
    assert [event["status"] for event in socket_response.json()["events"]] == [
        "queued",
        "claimed",
        "running",
        "done",
    ...
    assert terminal_response.json()["events"][-1]["status"] == "done"


def test_agents_crud_and_seats_status(api_client):
    ...
    assert created.json()["pane_id"] == "w1:p1"

    listed = api_client.get("/agents")
    assert listed.status_code == 200
    assert [agent["agent_id"] for agent in listed.json()] == [agent_id]

    removed = api_client.delete(f"/agents/{agent_id}")
    assert removed.status_code == 200
    assert removed.json()["status"] == "removed"

    seats = api_client.get("/seats")
    assert seats.status_code == 200
    assert seats.json()["seats"][0]["seat_id"] == "local-seat-codex"
