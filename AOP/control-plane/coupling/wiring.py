    ...
    *,
    ...
    if herdmaster_available:
        socket_exec = _socket_executor(
            fallback_queue_client, herdmaster_url, errors, token=herdmaster_token
        )
    else:
        if herdmaster_token:
        ...
        socket_exec = SocketExecutor(queue_client=fallback_queue_client, max_polls=1)
...
def _socket_executor(
    ...
    *,
    ...
    try:
        ...
        return SocketExecutor(queue_client=client, max_polls=1)
    except Exception as exc:
        errors.append(f"socket degraded: {exc}")
        return SocketExecutor(queue_client=fallback_queue_client, max_polls=1)
