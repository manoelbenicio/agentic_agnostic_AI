...
def checkin_task(
    ...
    agent_id: str = typer.Argument(..., help="Agent ID that is starting work."),
    config_path: Path | None = typer.Option(None, "--config", "-c", help="Path to config.toml."),
    json_output: bool = typer.Option(False, "--json", help="Emit JSON output."),
) -> None:
...
def complete_task(
    ...
    agent_id: str = typer.Argument(..., help="Agent ID that completed the task."),
    evidence: str = typer.Option(..., "--evidence", "-e", help="Evidence of delivery: file path, description, or screenshot path."),
    config_path: Path | None = typer.Option(None, "--config", "-c", help="Path to config.toml."),
    json_output: bool = typer.Option(False, "--json", help="Emit JSON output."),
) -> None:
...
def fail_task(
    ...
    agent_id: str = typer.Argument(..., help="Agent ID reporting the failure."),
    reason: str = typer.Option(..., "--reason", "-r", help="Precise description of what failed and why."),
    config_path: Path | None = typer.Option(None, "--config", "-c", help="Path to config.toml."),
    json_output: bool = typer.Option(False, "--json", help="Emit JSON output."),
) -> None:
...
async def _run_control_plane(config_path: Path | None, *, http_enabled: bool) -> None:
    ...
    watchdog = WatchdogEngine(adapter, agents, cfg.watchdog, bus_publisher=bus)
